...
int GraphTools::findShortestPath(Graph& graph, Vertex start, Vertex end)
{
    ...
    while (start != cur) {
        next = parent[cur];
        graph.setEdgeLabel(cur, next, "MINPATH");
        cur = next;
        distance++;
    }
...
void GraphTools::findMST(Graph& graph)
{
    ...
    // create DisjointSets to detect cycles
    DisjointSets dsets;
    dsets.addelements(numVertices);

    // add edges to MST, with no cycles
    while (added < numVertices - 1) {
        Edge edge = edges[idx];
        int forest1 = dsets.find(edge.source);
        int forest2 = dsets.find(edge.dest);
        if (forest1 != forest2) {
            graph.setEdgeLabel(edge.source, edge.dest, "MST");
            dsets.setunion(forest1, forest2);
            added++;
        }
        idx++;
    }

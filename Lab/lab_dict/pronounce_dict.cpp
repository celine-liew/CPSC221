...
{
    // Also, note that std::transform expects a string type, 
    // but the parameters of the homophones() 
    // function are declared const. Thus, you will need to 
    // make a copy of the strings given in order to transform them.
    /* Your code goes here! */
    // lookup->first  /* the 'first' item in the pair is the key */
    // lookup->second /* the 'second' item in the pair is the value */
    string tWord1 = word1;
    string tWord2 = word2;
    transform(tWord1.begin(), tWord1.end(), tWord1.begin(), ::toupper);
    transform(tWord2.begin(), tWord2.end(), tWord2.begin(), ::toupper);
    auto lookup1 = dict.find(tWord1);
    auto lookup2 = dict.find(tWord2);
